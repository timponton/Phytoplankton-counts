...
# Dates when significant difference was seen 
...
# Dates when sig diff was seen inclduing times 
small_time_date_sig <- ps_adf %>% 
  ...
  select(Date, p.adj.signif, p.adj, statistic) %>% 
  mutate(refe = 1700000)
...
# Dates when specific species were signficantly reduced between PS and ADF
sig_sp <- ps_adf %>% 
  filter(Date %in% small_time_date_sig$Date) %>% 
  select(Date, Site, Species, Classification, Correct.Original) %>%
  filter(Classification != "Various") %>% 
