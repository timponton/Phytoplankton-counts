...
RT <- df %>% 
  ...
  summarise(totalCellsS = sum(Correct.Original, na.rm = TRUE)) %>%
  ungroup() %>% 
  group_by(Date) %>% 
  summarise(totalCells = mean(totalCellsS, na.rm = TRUE)) %>% 
  mutate(RedTide = ifelse(totalCells <= 1000, "None detected", 
                          ifelse(totalCells >= 1001 & totalCells <= 5000, "Very Low (A)", 
                                 ifelse(totalCells >= 5001 & totalCells <= 10000, "Very Low (B)", 
                                        ifelse(totalCells >= 10001 & totalCells <= 50000, "Low (A)", 
...
df <- df %>% 
  left_join(RT, by = c("Date", "Time", "RowCode"))
...
df %>% 
  ...
  ggplot(., aes(x = dateTime, y = Correct.Original, col = Species)) +
  geom_point() +
  guides(col = FALSE) +
  scale_y_log10()
...
df %>% 
  ...
  geom_path() +
  scale_y_log10() +
  labs(x = "Date", y = "Daily mean count", title = "Daily averaged counts") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))


df %>% 
  filter(Site == "Primary sump", year == "2019") %>% 
  filter(Species != "Diatom", 
         Classification != "Various", 
         Classification != "Ciliate", 
         Classification != "Spirotrichea", 
         Classification != "Litostomatea") %>%
  group_by(dateTime, Classification) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>% 
  ggplot(., aes(x = dateTime, y = sumC, col = Classification)) +
  geom_point() +
  geom_path() +
  scale_y_log10() +
  labs(x = "Date", y = "Cells/L", title = "Different functional groups sampled in 2019") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
df %>% 
  filter(Site == "Primary sump", 
         Species == "Lingulodinium polyedra" | Species == "Gonyaulax polygramma" | Species == "Chaetoceros sp.", 
         year == "2019") %>% 
  ggplot(.,aes(x = dateTime, y = Correct.Original, col = Species)) +
  geom_point() +
  geom_path() +
  ...
  labs(x = "Date", y = "Daily mean count") +
  theme_classic()
...
df %>% 
  ...
  filter(Species != "Diatom", 
         ...
         Classification != "Spirotrichea", 
         Classification != "Litostomatea") %>% 
  group_by(Date, Species, Classification) %>% 
  summarise(meanC = mean(Correct.Original, na.rm = TRUE)) %>%
  ggplot(., aes(x = Date, y = meanC, col = Species)) +
  geom_point() +
  geom_path() +
  scale_y_log10() +
...
# Maximum count per day after total cells for each time point per day was calculated (Total cells per sample collected)
df %>% 
  group_by(Date, Time) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>% 
  group_by(Date) %>% 
  summarise(maxC = max(sumC)) %>% 
  ggplot(., aes(x = Date, y = maxC)) +
  geom_point() +
  geom_path() +
  scale_y_log10()
...
df %>% 
  ...
  geom_col(aes(fill = ifelse(n > 1 , "Red", "Blue"))) +
  guides(fill = FALSE) +
  theme_classic()
...
## Look at counts 
ggplotly(PS %>% 
  ggplot(., aes(x = Date, y = Correct.Original)) +
  geom_line())


ggplotly(df %>%
           filter(Counted.by. != "Lorna") %>% 
  group_by(dateTime) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>%
  ungroup() %>% 
...
dailyMeans <- PS %>% 
  ...
  summarise(meanBiomass = mean(Correct.Original, na.rm = TRUE)) %>% 
