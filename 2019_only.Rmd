...
nineTeen <- df %>% 
  filter(Date >= "2018-09-01" & Date <= "2019-12-31") %>%
...
ggplot(data = dailyMeans, aes(x = Date, y = meanBiomass)) +
  geom_point() +
  geom_line()
# Wrangle data ------------------------------------------------------------
...
DD_dom <- Sp_only %>% 
  ...
  pivot_longer(-c(months, month, year, Dominant), names_to = "Classification", values_to = "monthlyMean")
...
Diatom_dom <- DD_dom %>% 
  ...
  select(months, month, year, Dominant)
...
maximal <- nineTeenClean %>% 
  group_by(Date, months, month) %>% 
  summarise(totalCells = max(Correct.Original)) %>% 
  mutate(RedTide = ifelse(totalCells <= 1000, "None detected", 
                          ifelse(totalCells >= 1001 & totalCells <= 5000, "Very Low (A)", 
                                 ifelse(totalCells >= 5001 & totalCells <= 10000, "Very Low (B)", 
                                        ifelse(totalCells >= 10001 & totalCells <= 50000, "Low (A)", 
...
Time_count %>% 
  left_join(select(nineTeenClean, Date, months, month, year), by = "Date", keep = FALSE) %>% 
...
#total per time
all_max_cells_day <- nineTeenClean %>% 
  group_by(Date, Time, months, month, Species) %>% 
  summarise(total.per.time = sum(Correct.Original)) %>% 
  ungroup() %>% 
  group_by(Date, Time, months, month) %>% 
...
are <- all_max_cells_day %>% 
  left_join(select(nineTeenClean, Date, months, year), by = c("Date", "months"), keep = FALSE) %>%
  #filter(year != "2020") %>% 
...
RT.classNumbers_monthYear <- all_max_cells_day %>% 
  left_join(select(nineTeenClean, Date, months, year), by = "Date", keep = FALSE) %>%
  filter(year != "2020") %>% 
  group_by(months.x, month, year, FWC_mod) %>% 
  tally() %>% 
  arrange(desc(n))
...
# USe total data to plot total cells/l
totaL_density <- df %>% 
  filter(Date >= "2018-09-01" & Date <= "2019-12-31") %>%
  filter(Site == "Primary sump", 
         Correct.Original != 0) %>% 
  filter(Classification != "Ciliate", 
         Classification != "Spirotrichea", 
         Classification != "Litostomatea") %>% 
  group_by(Date, Time, months, month, year) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(Date, months, month, year) %>% 
  summarise(meanCells = mean(sumC, na.rm = TRUE)) %>% 
  view()
...
biomass.YM <- totaL_density %>%
  ...
  mutate(season = case_when(month == "1" ~ "Summer",
                            ...
                            month == "12" ~ "Summer",
                            TRUE ~ "Other")) %>%
  group_by(YEarMonth, season, year, months, month) %>% 
  summarise(monthlyMean = mean(meanCells, na.rm = TRUE), 
            monthlyN = n(),
            monthlySD = sd(meanCells, na.rm = TRUE), 
            monthlySE = sd(meanCells, na.rm = TRUE)/sqrt(n())) %>% 
  mutate(year_chopped = str_sub(year, 3)) %>% 
  mutate(ym = paste(abbreviate(months, minlength = 3, strict = TRUE, use.classes = FALSE), year_chopped))

biomass.YM$ym <- as.factor(biomass.YM$ym)

biomass.YM$ym <- factor(biomass.YM$ym, levels = c("Sep 18", "Oct 18", "Nov 18", "Dec 18",
                                                "Jan 19", "Feb 19", "Mar 19", "Apr 19", "May 19",
                                                "Jun 19", "Jul 19", "Aug 19", "Sep 19", "Oct 19", "Nov 19",
                                                "Dec 19"))
# selected are 
smaller <- ggplot(biomass.YM, aes(x = ym, y = monthlyMean, group = 1)) +
  geom_point(aes(col = season), size = 2.5) +
  geom_path(size = 1) +
  geom_errorbar(aes(col = season, ymin = monthlyMean - monthlySE, ymax = monthlyMean + monthlySE), width=0.2) +
  geom_text(x = "Nov 19", y = 70000, label = "B") +
  ...
  #ggtitle("Comparing the monthly mean phytoplankton counts over 16 consecutive months, with their standard error bars") +
  theme_classic() +
...
# Bigger overview
bigger <- ggplot(biomass.YM, aes(x = ym, y = monthlyMean, group = 1)) +
  geom_point(aes(col = season), size = 2.5) +
  geom_path(size = 1) +
  geom_errorbar(aes(col = season, ymin = monthlyMean - monthlySE, ymax = monthlyMean + monthlySE), width=0.2) +
  geom_text(x = "Nov 19", y = 800000, label = "A") +
  #scale_y_log10() +
  xlab("Months") +
  ylab("Cells/L") + 
  labs(col = "Season") +
  geom_hline(yintercept = 80000, linetype = "dashed") +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), 
...
Sp_only %>%
  ...
  mutate(season = case_when(month == "1" ~ "Summer",
                            ...
                            month == "12" ~ "Summer",
                            TRUE ~ "Other"), 
         other_season = case_when(month == "1" & year == "2019" ~ "Summer 2",
                            month == "2" & year == "2019" ~ "Summer 1",
                            month == "3" & year == "2019" ~ "Autumn 1",
                            month == "4" & year == "2019" ~ "Autumn 1",
                            month == "5" & year == "2019" ~ "Autumn 1",
                            month == "6" & year == "2019" ~ "Winter 1",
                            month == "7" & year == "2019" ~ "Winter 1",
                            month == "8" & year == "2019" ~ "Winter 1",
                            month == "9" & year == "2019" ~ "Spring 2",
                            month == "10" & year == "2019" ~ "Spring 2",
                            month == "11" & year == "2019" ~ "Spring 2",
                            month == "12" & year == "2019" ~ "Summer 2",
                            month == "12" & year == "2018" ~ "Summer 1",
                            month == "9" & year == "2018" ~ "Spring 1", 
                            month == "10" & year == "2018" ~ "Spring 1", 
                            month == "11" & year == "2018" ~ "Spring 1",
                            TRUE ~ "Other")) %>% 
  view()
...
sp_mat <- Sp_only %>%
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  mutate(season = case_when(month == "1" & year == "2019" ~ "Summer 2019",
                            month == "2" ~ "Summer",
                            month == "3" ~ "Autumn",
                            month == "4" ~ "Autumn",
                            month == "5" ~ "Autumn",
...
dd$ym <- as.factor(dd$ym)

dd$ym <- factor(dd$ym, levels = c("Sep 18", "Oct 18", "Nov 18", "Dec 18",
                                                "Jan 19", "Feb 19", "Mar 19", "Apr 19", "May 19",
                                                "Jun 19", "Jul 19", "Aug 19", "Sep 19", "Oct 19", "Nov 19",
                                                "Dec 19"))
...
richness.YM <- dd %>% 
  ...
  ggplot(., aes(x = as.factor(ym), y = monthlyRichness, group = 1)) +
  geom_point(size = 1.5, aes(col = season)) +
  #geom_line(size = 0.5) +
  geom_errorbar(aes(col = season, ymin = monthlyRichness - monthlyRichnessSE, ymax = monthlyRichness + monthlyRichnessSE), width=0.5, size = .9) +
  xlab("Months") +
  ylab("Richness") + 
  labs(col = "Season") +
...
evenness.YM <- dd %>% 
  ...
  ggplot(., aes(x = as.factor(ym), y = monthlyEvenness, group = 1)) +
  geom_point(size = 1.5, aes(col = season)) +
  #geom_line(size = 0.5) +
  geom_errorbar(aes(col = season,ymin = monthlyEvenness - monthlyEvennessSE, ymax = monthlyEvenness + monthlyEvennessSE), width=0.5, size = .9) + 
  xlab("Months") +
  ylab("J'") +
  labs(col = "Season") +
...
shannon.YM <- dd %>%
  ...
  ggplot(., aes(x = as.factor(ym), y = (monthlyshannon), group = 1)) +
  geom_point(size = 1.5, aes(col = season)) +
  #geom_line(size = 0.5) +
  geom_errorbar(aes(col = season, ymin = monthlyshannon - monthlyshannonSE, ymax = monthlyshannon + monthlyshannonSE), width=0.5, size = .9) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  xlab("Months") +
  ylab("H'") +
  #labs(col = "Season") +
...
# Evenness versus total abundance
dailyDesc %>% 
  filter(total_biomass < 500000) %>% 
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  mutate(season = case_when(month == "1" ~ "Summer",
                            month == "2" ~ "Summer",
                            month == "3" ~ "Autumn",
                            ...
                            month == "12" ~ "Summer",
                            TRUE ~ "Other")) %>% 
  ggplot(., aes(x = total_biomass, y = J)) +
...
month.rel <- as.data.frame(
  ...
  mutate(season = case_when(month == "1" ~ "Summer",
                            ...
                            month == "12" ~ "Summer",
                            TRUE ~ "Other")) %>% 
  filter(Species %in% c("Chaetoceros sp.", 
...
month.rel$ym <- as.factor(month.rel$ym)


month.rel %>% 
  mutate(ym = paste(abbreviate(months, minlength = 2, strict = TRUE), year_chopped)) %>% 
  select(YEarMonth.y, ym) %>% 
  distinct()

month.rel$ym <- factor(month.rel$ym, levels = c("Sep 18", "Oct 18", "Nov 18", "Dec 18",
                                                   "Jan 19", "Feb 19", "Mar 19", "Apr 19", "May 19",
                                                   "Jun 19", "Jul 19", "Aug 19", "Sep 19", "Oct 19", "Nov 19",
                                                   "Dec 19"))
...
month.rel$Species <- factor(month.rel$Species, levels = c("Chaetoceros sp.", "Gonyaulax polygramma", "Lingulodinium polyedra","Prorocentrum minimum", "Coscinodiscus granii"))


# month.rel %>%
#   #pivot_longer(-YEarMonth, names_to = "Species", values_to = "speAb") %>% 
#   group_by(year, season) %>% 
#   summarise(summing = sum(abun))
...
ggplot() + 
  geom_point(data = month.rel, 
             aes(x = ym, 
                 y = abun, group = 1, col = season), size = 2.5) +
  geom_line(data = month.rel, 
            aes(x = ym, 
                y = abun, group = 1, col = season), size = 2) +
  ...
  labs(col = "Season") +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  scale_y_continuous(breaks = seq(0, max(month.rel$abun), 15)) +
...
# plot yearly fluctuations for Diatoms
ggplot() + 
  geom_point(data = month.rel %>% 
               filter(Species %in% c("Chaetoceros sp.", "Coscinodiscus granii")), 
             aes(x = ym, 
                 y = abun, group = 1, col = season), size = 2.5) +
  geom_line(data = month.rel %>% 
               filter(Species %in% c("Chaetoceros sp.", "Coscinodiscus granii")), 
...
# plot yearly fluctuations for Dinoflagellates
ggplot() + 
  geom_point(data = month.rel %>% 
               filter(Species %in% c("Gonyaulax polygramma", "Lingulodinium polyedra","Prorocentrum minimum")), 
             aes(x = ym, 
                 y = abun, group = 1, col = season), size = 2.5) +
  geom_line(data = month.rel %>% 
               filter(Species %in% c("Gonyaulax polygramma", "Lingulodinium polyedra","Prorocentrum minimum")), 
...
# This might just be the same as above but for ggplotly
ggplotly(ggplot() + 
  geom_point(data = month.rel %>% 
               filter(Species %in% dsp$Species), 
             aes(x = YEarMonth, 
                 y = abun, group = 1, col = season), size = 1) +
  geom_line(data = month.rel %>% 
              filter(Species %in% dsp$Species), 
            aes(x = YEarMonth, 
                y = abun, group = 1, col = season), size = 1) +
...
#   top_n(1, meanRel)
# 
# # month.rel$Species <- factor(month.rel$Species, levels = c("Gonyaulax polygramma", "Lingulodinium polyedra", "Coscinodiscus granii", "Chaetoceros sp."))
# 
# # levels(month.rel$Species)
# 
# dsp <- month.rel %>% 
#   filter(Species %in% c("Chaetoceros sp.", 
#                         "Lingulodinium polyedra", 
...
#                         "Coscinodiscus granii"))
# 
# dsp$Species <- factor(dsp$Species, levels = c("Gonyaulax polygramma", "Lingulodinium polyedra", "Coscinodiscus granii", "Chaetoceros sp."))
...
species.list <- as.data.frame(
    ...
    as.matrix(
      Sp_only %>%
        ...
        mutate(season_exact = case_when(Date >= "2018-09-01" & Date <= "2018-11-30" ~ "Spring 2018",
                                        ...
                                        Date >= "2019-12-01" & Date <= "2019-12-31" ~ "Summer 2019 (December only)",
                                        TRUE ~ "Other")) %>% 
  ungroup() %>% 
  filter(Species != "Diatom") %>% 
  #mutate(Sp_abb = abbreviate(Species, 5, strict = FALSE)) %>% 
  ...
  relocate(Species, `Spring 2018`,`Summer 2018/2019`, `Autumn 2019`, `Winter 2019`, `Spring 2019`, `Summer 2019 (December only)`)

formattable(species.list)
write.csv(species.list, "Species_list.csv")
...
#'*[Stacked bar graph] of different phyto classes, by months, faceted by year ------------------*
# from Sp_Only df facet_wrap(~year)
bar_dinos <- Sp_only %>%
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>% 
  group_by(YEarMonth, Classification) %>% 
  summarise(monthlyMean = mean(meanCells, na.rm = TRUE)) %>% 
  ggplot(., aes(x = as.factor(YEarMonth), y = monthlyMean, fill = Classification)) +
  geom_bar(stat = "Identity", position = "fill", col = "Black") +
  ...
  ggtitle("Comparing the proportion of mean monthly counts between phytoplankton 
          functional groups over 16 consecutive months") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
#   ggplot(., aes(x = as.factor(YEarMonth), y = monthlyMean, group = Classification)) +
#   geom_point(aes(col = Classification)) +
#   geom_path(aes(col = Classification)) +
#   geom_errorbar(aes(col = Classification, ymin = monthlyMean - monthlySE, ymax = monthlyMean + monthlySE), width=0.2) +
...
line_dinos <- nineTeenClean %>%
  group_by(Date, Time, Classification, month, months, year) %>%
  summarise(sumClas = sum(Correct.Original, na.rm = TRUE)) %>% 
  #filter(Classification != "Dictyochophyceae") %>% 
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  mutate(season = case_when(month == "1" ~ "Summer",
                            month == "2" ~ "Summer",
                            ...
                            month == "12" ~ "Summer",
                            TRUE ~ "Other")) %>%
  group_by(YEarMonth, season, Classification, year, months, month) %>% 
  summarise(monthlyMean = mean(sumClas, na.rm = TRUE), 
            monthlyN = n(),
            monthlySD = sd(sumClas, na.rm = TRUE), 
            monthlySE = sd(sumClas, na.rm = TRUE)/sqrt(n())) %>%
  mutate(year_chopped = str_sub(year, 3)) %>% 
  mutate(ym = paste(abbreviate(months, minlength = 3, strict = TRUE, use.classes = FALSE), year_chopped))


line_dinos$ym <- as.factor(line_dinos$ym)

line_dinos$ym <- factor(line_dinos$ym, levels = c("Sep 18", "Oct 18", "Nov 18", "Dec 18",
                                                "Jan 19", "Feb 19", "Mar 19", "Apr 19", "May 19",
                                                "Jun 19", "Jul 19", "Aug 19", "Sep 19", "Oct 19", "Nov 19",
                                                "Dec 19"))
...
ggplot(line_dinos, aes(x = ym, y = monthlyMean, group = Classification)) +
  geom_point(aes(col = Classification)) +
  geom_path(aes(col = Classification), size = 1.1) +
  geom_errorbar(aes(col = Classification, ymin = monthlyMean - monthlySE, ymax = monthlyMean + monthlySE), width=0.2) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  #facet_wrap(~Classification, ncol = 1) +
  scale_y_log10() +
  ...
  #ggtitle("Comparing the log10 monthly mean counts of phytoplankton functional groups between 16 consecutive months, with their standard error bars") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5), 
        text=element_text(size=17), 
axis.text.x = element_text(face="bold", 
...
ggplot(are %>% 
         mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")), aes(x = as.factor(YEarMonth), y = n, fill = FWC_mod)) +
  geom_bar(stat = "Identity", position = "fill", col = "black") +
  #facet_wrap(~year, ncol = 2) +
  xlab("Months") +
...
# Number of samples collected vs cells counted, coloured by season

samples <- nineTeenClean %>% 
  group_by(Date, Time, months, month, year) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>% 
  distinct() %>% 
  group_by(Date, months, month, year) %>%
...
nineTeenClean %>% 
  ...
  group_by(Date, months, month, year) %>% 
  summarise(meanC = mean(sumC, na.rm = TRUE)) %>% 
  left_join(samples, by = c("Date", "month", "months", "year"), keep = FALSE) %>%
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  mutate(season = case_when(month == "1" ~ "Summer",
                            month == "2" ~ "Summer",
                            month == "3" ~ "Autumn",
                            ...
                            month == "12" ~ "Summer",
                            TRUE ~ "Other")) %>% 
  ggplot(.,aes(x = Date, y = meanC, group = 1)) +
  geom_point(aes(size = n, col = season)) +
  geom_line(aes(col = season)) +
  scale_y_log10(labels=function(x) format(x, big.mark = ",", scientific = FALSE)) +
  scale_x_date(date_labels = "%b%y", date_breaks = "1 month", guide = guide_axis(n.dodge = 2)) +
  xlab("Date") +
  ylab("Cells/L (Log10)") + 
  labs(col = "Season", size = "Samples Collected") +
  #ggtitle("Relationship between Log10 Cells/L counted and number of samples collected per day over 16 consecutive months") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5), 
        text=element_text(size=17), 
axis.text.x = element_text(face="bold", 
...
df %>% 
  ...
  ungroup() %>% 
  group_by(Date) %>% 
  summarise(monthlyMean = mean(sumC, na.rm = TRUE), 
            monthlyN = n(),
            monthlySD = sd(sumC, na.rm = TRUE), 
            monthlySE = sd(sumC, na.rm = TRUE)/sqrt(n()))
...
march_feb_total <- nineTeenClean %>% 
  group_by(Date, Time, months, month, year) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>%
  filter(months %in% c("March", "February")) %>% 
  group_by(Date, months, month, year) %>% 
  summarise(meanC = mean(sumC, na.rm = TRUE))
...
# Kruskal W for March and Feb
kruskal.test(sumC ~ months, data = march_feb_total)


posthoc.kruskal.dunn.test(march_feb_total$sumC, march_feb_total$month, dist = "Tukey", p.adjust.methods = "bonferroni")

####### Functional group differences ####

pre_bloom <- nineTeenClean %>%
  group_by(Date, Time, Classification, month, months, year) %>%
  summarise(sumClas = sum(Correct.Original, na.rm = TRUE)) %>% 
  #filter(Classification != "Dictyochophyceae") %>% 
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  ...
  summarise(monthlyMean = mean(sumClas, na.rm = TRUE), 
            ...
            monthlySD = sd(sumClas, na.rm = TRUE), 
            monthlySE = sd(sumClas, na.rm = TRUE)/sqrt(n())) %>% 
  mutate(bloom = case_when(YEarMonth == "Sep 2018" ~ "Pre", 
                           YEarMonth == "Oct 2018" ~ "Pre",
                           YEarMonth == "Nov 2018" ~ "Pre",
                           YEarMonth == "Dec 2018" ~ "Bloom",
                           YEarMonth == "Jan 2019" ~ "Bloom",
                           YEarMonth == "Feb 2019" ~ "Bloom",
                           YEarMonth == "Mar 2019" ~ "Bloom",
                           YEarMonth == "Apr 2019" ~ "Bloom", 
                           YEarMonth == "May 2019" ~ "Bloom", 
                           YEarMonth == "Jun 2019" ~ "Bloom",
                           YEarMonth == "Jul 2019" ~ "Post",
                           YEarMonth == "Aug 2019" ~ "Post",
                           YEarMonth == "Sep 2019" ~ "Post",
                           YEarMonth == "Oct 2019" ~ "Post",
                           YEarMonth == "Nov 2019" ~ "Post",
                           YEarMonth == "Dec 2019" ~ "Post",
                           TRUE ~ "Other")) %>% 
  filter(bloom == "Post") %>% 
  select(Classification, monthlyMean)


# Kruskal W for March and Feb
kruskal.test(monthlyMean ~ Classification, data = pre_bloom)
...
#### Richness (New Sep-Dec) ####

richness_stats <- dailyDesc %>% 
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  mutate(season = case_when(month == "1" ~ "Summer",
                            month == "2" ~ "Summer",
                            month == "3" ~ "Autumn",
...
# test using Kruskal
kruskal.test(S ~ year, data = spring_richness)

# Pairwise tests
# ties are present so did not work
pairwise.wilcox.test(richness_stats$S, richness_stats$YEarMonth, p.adjust.method = "bonf")



# Posthoc using kruskal Dunn -> Works
posthoc.kruskal.dunn.test(richness_stats$S, richness_stats$season, dist = "Tukey", p.adjust.methods = "bonferroni")
...
# Correlation between total cells and evenness
J_dens_corr <- totaL_density %>% 
  left_join(dailyDesc %>% 
              select(Date, J), by = "Date")

cor.test(J_dens_corr$meanCells, J_dens_corr$J, method = "spearman", alternative = "g")

cor(J_dens_corr$meanCells, J_dens_corr$J, method = c("pearson", "kendall", "spearman"))


#### Species diversity ####
shannon_spring <- dailyDesc %>%
  mutate(YEarMonth = as.yearmon(paste(year, month), "%Y %m")) %>%
  mutate(season = case_when(month == "1" ~ "Summer",
                            month == "2" ~ "Summer",
                            month == "3" ~ "Autumn",
...
# test using Kruskal
kruskal.test(shannon ~ year, data = shannon_spring)
...
# test using Kruskal
kruskal.test(n ~ month, data = samples)

# Pairwise tests
# ties are present so did not work
pairwise.wilcox.test(samples$n, samples$month, p.adjust.method = "bonf")


# Posthoc using kruskal Dunn -> Works
posthoc.kruskal.dunn.test(samples$n, samples$month, dist = "Tukey", p.adjust.methods = "bonferroni")
