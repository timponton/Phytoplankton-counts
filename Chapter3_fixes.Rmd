...
df %>% 
  ...
  filter(delta != "NA", 
         delta < 28140000,
         `Primary sump` < 5000000,
         delta > 0, 
         `After Drumfilter` > 0) %>%
  left_join(sig_dates, by = "Date") %>% 
  ggplot(., aes(x = `Primary sump`, y = delta)) +
  geom_point(aes(col = p.adj.signif)) +
  geom_smooth(method = lm) +
  scale_x_log10() +
  scale_y_log10()
  

## different signficance levels (23 Nov)

sig_dates <- ps_adf %>% 
  #filter(Date == "2019-03-02") %>% 
  select(Site, Correct.Original, Date) %>% 
  group_by(Date) %>% 
  dunn_test(Correct.Original ~ Site, p.adjust.method = "bonferroni") %>% 
  select(Date, p.adj.signif)
...
df %>% 
  ...
  ggplot(., aes(x = `Primary sump`, y = `After Drumfilter`, col = months, size = delta)) +
  geom_point() +
  #scale_y_log10() +
  #scale_x_log10() +
  facet_wrap(~ p.adj.signif, scales = "free")
...
ps_adf %>% 
  pivot_wider(names_from = Site, values_from = Correct.Original, 
              values_fill = list(Correct.Original = 0)) %>%
  mutate(delta = `Primary sump` - `After Drumfilter`) %>% 
  #filter(delta > 0) %>%
  view()
  select(Date, Time, Species, Classification, `Primary sump`, `After Drumfilter`) %>% 
  pivot_longer(cols = `Primary sump`:`After Drumfilter`, names_to = "Site", values_to = "cells") %>% 
  ggplot(., aes(x = Date, y = cells, col = Site)) +
  geom_line()
...
ps_adf %>% 
  ...
  geom_point(aes(x = Date, y = mean_cells, col = Site, group = Site), position = pd) +
  geom_errorbar(aes(ymin = mean_cells - se_cells, ymax = mean_cells + se_cells, col = Site, group = Site), width=0.2, position = pd) +
  #geom_line(aes(x = Date, y = mean_cells, col = Site, group = Site), position = pd) +
...
## Pivot wider to see if species are in PS and ADF on the same day 
ggplotly(Sp.only_filter %>% 
  pivot_wider(names_from = Site, values_from = meanCells, values_fill = list(meanCells = 0)) %>% 
  mutate(delta = `Primary sump` - `After Drumfilter`) %>% 
  filter(delta != 0) %>% 
  ggplot(.,aes(x = Date, y = delta, col = Classification)) +
  geom_point())


basis <- Sp.only_filter %>% 
  pivot_wider(names_from = Site, values_from = meanCells, values_fill = list(meanCells = 0)) %>% 
  mutate(delta = `Primary sump` - `After Drumfilter`) %>% 
  filter(delta > 0) %>% 
  select(-delta) %>% 
...
# correct the order of sites (PS before ADf)
basis$Site <- as.factor(basis$Site)
basis$Site <-  factor(basis$Site, 
                       levels = c("Primary sump", "After Drumfilter"))


ggplot(data = basis, aes(x = Date, y = cells, col = Site)) +
  geom_line() +
  scale_y_log10()

basis %>% 
  group_by(Date, months, month, Site) %>%
  summarise(mean_cells = mean(cells, na.rm = TRUE),
            n_cells = n(),
            sd_cells = sd(cells, na.rm = TRUE),
            se_cells = sd(cells, na.rm = TRUE)/sqrt(n())) %>%
  ggplot(., aes(x = Date, y = mean_cells, col = Site, group = Site)) +
  geom_point(aes(x = Date, y = mean_cells, col = Site, group = Site)) +
...
Sp.only_filter %>% 
  filter(Date %in% basis$Date & Species %in% basis$Species) %>% 
  group_by(Date, months, month, Site) %>%
...
# Wilcox enitre study period
main_df %>% 
  group_by(Date, Time, Site, months, month) %>% 
  summarise(sumC = sum(Correct.Original, na.rm = TRUE)) %>% 
  ungroup() %>% 
  wilcox_test(sumC ~ Site, p.adjust.method = "bonferroni") %>% 
  view()

# Wilcox method monthly
total_abun %>% 
  ungroup() %>% 
  select(Site, meanC, months) %>% 
  group_by(months) %>% 
  wilcox_test(meanC ~ Site, p.adjust.method = "bonferroni")
...
main_df %>% 
  ...
  summarise(meanC = mean(sumC, na.rm = TRUE)) %>% 
  ungroup() %>% 
  wilcox_test(meanC ~ Site, p.adjust.method = "none")
...
main_df %>% 
  ...
  #summarise(meanC = mean(sumC, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(months, Classification) %>% 
  wilcox_test(sumC ~ Site, p.adjust.method = "none") %>% 
  view()
