...
nineTeenClean %>% 
  group_by(Date, Time, year, months, month) %>% 
  summarise(meanCells = sum(Correct.Original, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(year, months, month) %>% 
  ...
  ylab("Cells/L") + 
  labs(col = "Year") +
  ggtitle("Comparing the monthly mean phytoplankton counts between two consecutive years, with their standard error bars") +
  theme_classic() +
...
ggplot(left_join(max_cells_day, Time_count, by = "Date") %>% 
         filter(count_times > 0, Date >= "2018-09-01"), aes(x = log(totalCells), y = count_times)) +
  geom_point(aes(col = FWC_mod)) +
