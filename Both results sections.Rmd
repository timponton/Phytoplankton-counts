...
biomass.YM <- Sp_only %>% 
  ...
  mutate(months = substr(months, 1, 3)) %>% 
...
### These are added to try and fix aesthetics - rerun above for proper individual plots of each descriptor
#'*(1) ------------------*
# Richness
richness.YM <- dailyDesc %>% 
  group_by(months, month, year, Season) %>% 
...
month.rel %>% 
  ...
  mutate(months = substr(months, 1, 3)) %>% 
  ggplot(., aes(x = reorder(months, -month), 
                y = abun, group = year, col = year), size = 1) +
  geom_line() +
  geom_point() +
  facet_wrap(~Species, ncol = 7, scales = "free_y", labeller = (groupwrap = label_wrap_gen(10))) +
  coord_flip() +
  theme_classic() +
  xlab("Months") +
...
# Dendrogram of similarity between months

dd <- vegdist(monthly.species_wide, method = "bray") * 100
hc <- hclust(dd, method = "complete")

fviz_dend(hc, cex = .75,
          main = "",
          xlab = "Months", ylab = "Similarity %", sub = "", 
...
## Use are df for red tide classes#
## This is just to abbreviate months
are %>% 
  mutate(months = substr(months, 1, 3)) %>% 
  ...
  facet_wrap(~year, ncol = 1) +
  xlab("Months") +
  ylab("Class proportion per month") + 
  labs(fill = "Red Tide class") +
  #ggtitle("The proportion of different red tide classes per month over the three years") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
left_join(max_cells_day, Time_count, by = "Date") %>% 
  ...
  #ggtitle("Investigating how sampling frequency varied in relation to red tide severity in 2019") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
#  geom_smooth(method = "loess", se = FALSE) +
  ...
  #ggtitle("Relationship between Cells/L counted and number of samples collected per day, when different Red tide severity classes were experienced") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
#  geom_line() +
  geom_errorbar(aes(ymin = mean_cells - se_cells, ymax = mean_cells + se_cells), width=0.2, position=position_dodge(0.25)) +
  xlab("Months") +
  ylab("Cells/L") + 
  labs(fill = "Site") +
#  ggtitle("") +
...
rich <- dailydesc_filter %>% 
  group_by(months, month, year, YEarMonth, Site) %>% 
  summarise(mean_S = mean(S, na.rm = TRUE), 
            n_cells = n(),
            sd_cells = sd(S, na.rm = TRUE), 
            se_cells = sd(S, na.rm = TRUE)/sqrt(n())) %>% 
  ggplot(., aes(x = reorder(months, month), y = mean_S, col = Site, group = Site)) +
  geom_point(position=position_dodge(0.25)) +
#  geom_line() +
  geom_errorbar(aes(ymin = mean_S - se_cells, ymax = mean_S + se_cells), width=0.2, position=position_dodge(0.25)) +
  xlab("Months") +
  ylab("Richness") + 
  labs(fill = "Site") +
#  ggtitle("") +
...
even <- dailydesc_filter %>% 
  group_by(months, month, year, YEarMonth, Site) %>% 
  summarise(mean_J = mean(J, na.rm = TRUE), 
            n_cells = n(),
            sd_cells = sd(J, na.rm = TRUE), 
            se_cells = sd(J, na.rm = TRUE)/sqrt(n())) %>% 
  ggplot(., aes(x = reorder(months, month), y = mean_J, col = Site, group = Site)) +
  geom_point(position=position_dodge(0.25)) +
#  geom_line() +
  geom_errorbar(aes(ymin = mean_J - se_cells, ymax = mean_J + se_cells), width=0.2, position=position_dodge(0.25)) +
  xlab("Months") +
  ylab("J'") + 
  labs(fill = "Site") +
#  ggtitle("") +
...
shan <- dailydesc_filter %>% 
  group_by(months, month, year, YEarMonth, Site) %>% 
  summarise(mean_shannon = mean(shannon, na.rm = TRUE), 
            n_cells = n(),
            sd_cells = sd(shannon, na.rm = TRUE), 
            se_cells = sd(shannon, na.rm = TRUE)/sqrt(n())) %>% 
  ggplot(., aes(x = reorder(months, month), y = mean_shannon, col = Site, group = Site)) +
  geom_point(position=position_dodge(0.25)) +
#  geom_line() +
  geom_errorbar(aes(ymin = mean_shannon - se_cells, ymax = mean_shannon + se_cells), width=0.2, position=position_dodge(0.25)) +
  xlab("Months") +
  ylab("H'") + 
  labs(fill = "Site") +
#  ggtitle("") +
...
### Put all together for drafts

# diversity 
bio
rich/even/shan

## Dino Diatoms BETWEEN SITES #####
--------------------------------

Sp.only_filter %>% 
  group_by(Site, months, month, Classification) %>% 
...
# Plot as point error bars method





## Use SD to plot errorbars (This works, and shows no significant difference)
# MEasurements have been log10 transformed
...
Sp.only_filter %>% 
  ...
  geom_point() +
  #  geom_line() +
  geom_errorbar(aes(ymin = mean_cells - sd_cells, ymax = mean_cells + sd_cells), width=0.2) +
...
# Use SE (where some error-bars do not overlap but p-value isn't significant)
...
Sp.only_filter %>% 
  group_by(Site, months, month, Species, Classification) %>% 
  filter(Species %in% "Prorocentrum micans") %>% 
  summarise(mean_cells = mean(meanCells, na.rm = TRUE), 
            n_cells = n(),
            sd_cells = sd(meanCells, na.rm = TRUE), 
            se_cells = sd(meanCells, na.rm = TRUE)/sqrt(n()))
...
# Check if significance between sites for each classification for each month manually
# Manually change class and month to check

Sp.only_filter %>% 
  group_by(Site, months, month, Classification) %>% 
  filter(Classification %in% "Dinoflagellate") %>% 
  filter(months == "April") %>% 
  ungroup() %>% 
  select(meanCells, Site) %>% 
  dunn_test(meanCells ~ Site, p.adjust.method = "bonferroni") 
